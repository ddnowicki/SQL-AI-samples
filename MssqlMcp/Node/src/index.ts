...
export async function createSqlConfig(): Promise<{ config: sql.config, token: string, expiresOn: Date }> {
  ...
  // Check if SQL Server authentication credentials are provided
  ...
  if (sqlUsername && sqlPassword) {
    ...
    return {
      ...
      token: '', // No token needed for SQL authentication
      expiresOn: new Date(Date.now() + 24 * 60 * 60 * 1000) // 24 hours from now (arbitrary for SQL auth)
    };
  } else {
...
async function ensureSqlConnection() {
  // Check if we're using SQL Server authentication (no token expiry concerns)
  const sqlUsername = process.env.SQL_USERNAME;
  const sqlPassword = process.env.SQL_PASSWORD;
  
  if (sqlUsername && sqlPassword) {
    // For SQL Server authentication, just check if pool is connected
  ...
  // Azure AD authentication logic (existing)
